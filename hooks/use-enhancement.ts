...
export function useEnhancement(options: UseEnhancementOptions = {}) {
  ...
  const enhance = useCallback(
    async (documentId: string, preferences?: EnhancementPreferences) => {
      setIsEnhancing(true)
      setProgress(0)
      setError(null)
      setResult(null)
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Enhancement failed")
        setError(error)
        onError?.(error)
      } finally {
  ...
  const pollEnhancementStatus = useCallback(
