...
export function useOptimisticUpdate() {
  const addOptimisticUpdate = useDashboardStore(
    (state) => state.addOptimisticUpdate
  )
  const removeOptimisticUpdate = useDashboardStore(
    (state) => state.removeOptimisticUpdate
  )
  const rollbackOptimisticUpdate = useDashboardStore(
  ...
  const deleteDocument = useCallback(
    async (documentId: string, serverAction: (id: string) => Promise<void>) => {
      const updateId = uuidv4()
      const update: OptimisticUpdate<Document> = {
        id: updateId,
        type: "delete",
