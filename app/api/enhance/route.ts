...
async function getEnhancementStatusHandler(
  request: NextRequest,
  context?: APIRequestContext
) {
  const requestId =
  ...
  try {
    ...
    if (!documentId) {
      ...
      const enhancementService = new EnhancementService()
      const enhancements = await enhancementService.listUserEnhancements(
        user.id
      )
    ...
    const enhancementService = new EnhancementService()
    const status = await enhancementService.getEnhancementStatus(
      documentId,
      user.id
    )
