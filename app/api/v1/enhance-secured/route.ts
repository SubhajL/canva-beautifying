...
const enhanceHandler = async (
  ...
  const userId = auth
    ? context?.user?.id || context?.apiKey?.userId
    : await authenticateRequest(request).then((r) => r.userId)
  ...
  asyncHandler(
    documentRoute(
      enhanceHandler,
      {
        ...
        description:
          "Enhanced version of the enhance endpoint with dual rate limiting to prevent authenticated users from bypassing rate limits",
        tags: ["Enhancements"],
