...
export const POST = documentRoute(
  postHandler,
  {
    ...
    description:
      "Export an enhanced document in various formats (PNG, JPG, PDF, Canva). Supports both single document and batch exports. Exports can be processed synchronously or asynchronously.",
    tags: ["export"],
    security: [{ bearerAuth: [] }],
  },
  requestBody(exportCombinedSchema, {
    ...
    response(200, "Synchronous export completed successfully", {
      schema: z.object({
        ...
        exportUrl: z
          .string()
          .url()
          .describe("Download URL for the exported file"),
        ...
        dimensions: z
          ...
          .optional()
          .describe("Dimensions for image exports"),
    ...
    response(201, "Async export job queued successfully", {
      schema: z.union([
        ...
        z.object({
          ...
          jobIds: z
            .array(z.string())
            .describe("Job IDs for batch export tracking"),
    ...
    if (validatedData.async) {
      ...
      const job = await exportQueue.add(
        "export-document",
        {
          ...
          exportFormat: validatedData.format as
            | "png"
            | "pdf"
            | "canva"
            | "pptx",
          exportSettings: {
            quality:
              validatedData.options?.quality === 100
                ? "high"
                : validatedData.options?.quality === 50
                  ? "standard"
                  : "standard",
            includeReport: false,
            watermark: false,
...
const exportQuerySchema = z.object({
  ...
  documentId: z
    ...
    .uuid()
    .optional()
    .describe("Document ID to check export progress"),
})
...
export const GET = documentRoute(
  getHandler,
  {
    ...
    description:
      "Check the status of an export job, get export progress for a document, or retrieve export history for the authenticated user.",
    tags: ["export"],
    security: [{ bearerAuth: [] }],
  },
  queryParams(
    ...
    response(200, "Successfully retrieved export information", {
      schema: z.union([
        ...
        z.object({
          ...
          status: z
            .string()
            .describe("Job status (waiting, active, completed, failed)"),
