...
import { authenticateRequest, generateRequestId } from "@/lib/api/middleware"
import {
  successResponse,
  errorResponse,
  apiErrors,
...
const getHandler = async (request: NextRequest, { params }: RouteParams) => {
  ...
  try {
    ...
    if (
      ...
      const queues = [QUEUE_NAMES.ENHANCEMENT, QUEUE_NAMES.EXPORT]
      ...
      for (const queueName of queues) {
        ...
        const jobIndex = jobs.findIndex(
          (job) =>
            job.data.enhancementId === id ||
            job.data.documentId === enhancement.document_id
        )
...
export const GET = documentRoute(
  getHandler,
  {
    ...
    description:
      "Retrieve the current status and details of a document enhancement job. Includes queue position and estimated wait time for pending jobs.",
    tags: ["enhance"],
    security: [{ bearerAuth: [] }],
    parameters: [
      {
      ...
      schema: z.object({
        ...
        data: z.object({
          ...
          currentStage: z
            .string()
            .optional()
            .describe("Current processing stage"),
          queuePosition: z
            .number()
            .optional()
            .describe("Position in processing queue"),
          estimatedWaitTime: z
            .number()
            .optional()
            .describe("Estimated wait time in seconds"),
          ...
          result: z
            .object({
              enhancedFileUrl: z
                .string()
                .url()
                .describe("Enhanced document URL"),
              thumbnailUrl: z
                .string()
                .url()
                .nullable()
              ...
              enhancementsApplied: z
                .array(z.string())
                .describe("List of enhancements applied"),
              processingTime: z
                .number()
                .nullable()
              ...
              reportUrl: z
                ...
                .url()
                .nullable()
                .describe("Enhancement report URL"),
            })
            .optional()
            .describe("Result data (only when completed)"),
...
const deleteHandler = async (request: NextRequest, { params }: RouteParams) => {
  const requestId = generateRequestId()
  ...
  try {
      ...
      for (const job of jobs) {
        if (
          ...
          try {
            ...
            console.error(
              `Failed to remove job ${job.id} from queue ${queueName}:`,
              error
            )
          }
        }
    ...
    if (updateError) {
      throw new ApiError(
        "DATABASE_ERROR",
        "Failed to update enhancement status",
        500
      )
    }
...
export const DELETE = documentRoute(
  deleteHandler,
  {
    ...
    description:
      "Cancel a pending or processing enhancement job. Only works for jobs that have not completed yet.",
    tags: ["enhance"],
    security: [{ bearerAuth: [] }],
    parameters: [
      {
    ...
    response(200, "Enhancement cancelled successfully", {
      schema: z.object({
        ...
        data: z.object({
          ...
          jobRemoved: z
            .boolean()
            .describe("Whether the job was removed from queue"),
        }),
      }),
    }),
    response(
      400,
      "Bad request - Cannot cancel enhancement with current status"
    ),
    response(401, "Unauthorized - Invalid or missing authentication"),
    response(403, "Forbidden - Enhancement belongs to another user"),
    response(404, "Enhancement not found"),
