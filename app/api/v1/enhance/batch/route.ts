...
} from "@/lib/utils/create-safe-error"
...
async function batchEnhanceHandler(
  request: NextRequest,
  ...
  try {
    ...
    if (
      ...
      const error = createValidationError(
        "documentIds",
        "Document IDs array is required and must not be empty"
      )
    ...
    const batchLimit =
      BATCH_LIMITS[tier as keyof typeof BATCH_LIMITS] || BATCH_LIMITS.free
    ...
    const { error: batchError } = await supabase
      ...
      .insert({
        ...
        status:
          successfulCount === documentIds.length ? "processing" : "partial",
      })
...
async function getBatchStatusHandler(
  ...
  try {
    ...
    const processing = statuses.filter((s) =>
      ["processing", "analyzing"].includes(s)
    ).length
