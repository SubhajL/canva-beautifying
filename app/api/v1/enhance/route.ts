...
const enhanceHandler = asyncHandler(
  async (request: NextRequest, context?: APIRequestContext) => {
    ...
    if (uploadTraceId && uploadSpanId) {
      const pipelineSpan = (pipelineTracer as any).pipelineSpans.get(
        enhancementId
      )?.span
      if (pipelineSpan) {
    ...
    await enhancementQueue.add("process-enhancement", jobData, {
      priority:
        enhanceRequest.priority === "high"
          ? 1
          : enhanceRequest.priority === "low"
            ? 3
            : 2,
    ...
    pipelineTracer.addPipelineEvent(enhancementId, "enhancement.queued", {
...
const postHandler = documentRoute(
  enhanceHandler,
  {
    ...
    description:
      "Upload a document for AI-powered enhancement. The document will be queued for processing and you can check the status using the returned enhancement ID.",
    tags: ["enhance"],
    security: [{ bearerAuth: [] }],
  },
  requestBody(
    z.object({
      file: z
        .instanceof(File)
        .describe("Document file to enhance (PDF, PNG, JPG, WEBP, PPT, PPTX)"),
      settings: z
        .string()
        .describe("JSON string containing enhancement settings"),
    }),
    {
...
const getHandler = asyncHandler(async (request: NextRequest) => {
  ...
  const formattedEnhancements = enhancements?.map((enhancement) => ({
    ...
    links: {
      ...
      cancel:
        enhancement.status === "pending" || enhancement.status === "processing"
...
export const GET = documentRoute(
  getHandler,
  {
    ...
    description:
      "Retrieve a paginated list of document enhancements for the authenticated user",
    tags: ["enhance"],
    security: [{ bearerAuth: [] }],
  },
  queryParams(
          ...
          z.object({
            ...
            links: z.object({
              ...
              cancel: z
                .string()
                .optional()
                .describe("URL to cancel (if applicable)"),
            }),
          })
        ),
        pagination: z.object({
