...
const createWebhookSchema = z.object({
  url: z
    ...
    .refine((url) => {
      ...
      return (
        process.env.NODE_ENV === "development" || parsed.protocol === "https:"
      )
...
const webhookQuerySchema = z.object({
  ...
  events: z
    .string()
    .transform((val) => val.split(",") as WebhookEventType[])
    .optional(),
})
  ...
  withRateLimit({ maxRequests: 100 })(
    withValidation({
      ...
      try {
        ...
        const webhooks = await webhookManager.listWebhooks(context.user!.id, {
...
export const GET = documentRoute(
  getHandler,
  {
    ...
    description:
      "Retrieve all webhooks configured for the authenticated user. Supports filtering by active status and event types.",
    tags: ["webhooks"],
    security: [{ bearerAuth: [] }],
  },
  queryParams(webhookQuerySchema, "Query parameters for filtering webhooks"),
  responses(
        ...
        data: z.object({
          webhooks: z.array(
            z.object({
              ...
              events: z
                .array(z.string())
                .describe("Events this webhook is subscribed to"),
              headers: z
                .record(z.string())
                .optional()
                .describe("Custom headers to send with webhook"),
              is_active: z
                .boolean()
                .describe("Whether the webhook is currently active"),
              ...
              last_triggered_at: z
                ...
                .datetime()
                .nullable()
                .describe("Last successful trigger timestamp"),
              retry_policy: z
                .object({
  ...
  withRateLimit({ maxRequests: 10 })(
    withValidation({
      ...
      try {
        ...
        const webhook = await webhookManager.createWebhook(context.user!.id, {
          url: validated.url,
...
export const POST = documentRoute(
  postHandler,
  {
    ...
    description:
      "Create a new webhook to receive notifications for specified events. Webhook URLs must use HTTPS in production.",
    tags: ["webhooks"],
    security: [{ bearerAuth: [] }],
  },
  requestBody(createWebhookSchema, {
