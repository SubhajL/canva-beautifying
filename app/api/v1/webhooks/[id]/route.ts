      ...
      schema: z.object({
        ...
        data: z.object({
          ...
          last_triggered_at: z
            ...
            .datetime()
            .nullable()
            .describe("Last trigger timestamp"),
          retry_policy: z
            .object({
...
export const PUT = documentRoute(
  putHandler,
  {
    ...
    description:
      "Update an existing webhook configuration. Any fields not provided will remain unchanged.",
    tags: ["webhooks"],
    security: [{ bearerAuth: [] }],
    parameters: [
      {
  ...
  requestBody(updateWebhookSchema, {
...
const deleteHandler = withAuth(
  withRateLimit({ maxRequests: 50 })(
    async (request: NextRequest, context, { params }: RouteParams) => {
      try {
        await webhookManager.deleteWebhook(context.user!.id, params.id)
