    ...
    withValidation({
      ...
      documentRoute(
        getWebhookStatsHandler,
        {
          ...
          description:
            "Retrieves aggregated statistics for webhook deliveries across all webhooks or for a specific webhook",
          tags: ["Webhooks"],
          parameters: [
            {
              name: "webhookId",
...
function calculateWebhookStats(logs: any[], period: string) {
  ...
  logs.forEach((log) => {
    ...
    if (isSuccess) {
      ...
      if (log.delivered_at && log.created_at) {
        const responseTime =
          new Date(log.delivered_at).getTime() -
  ...
  summary.successRate =
    summary.totalDeliveries > 0
      ? Math.round(
          (summary.successfulDeliveries / summary.totalDeliveries) * 100
        )
      : 0
  ...
  Object.values(byEventType).forEach((event) => {
    event.successRate =
      event.totalDeliveries > 0
        ? Math.round((event.successfulDeliveries / event.totalDeliveries) * 100)
        : 0
  })
...
function getTimeSeriesKey(date: Date, period: string): string {
  ...
  switch (period) {
    case "hour":
      const minute = String(Math.floor(date.getMinutes() / 10) * 10).padStart(
        2,
        "0"
      )
