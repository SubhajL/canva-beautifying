...
const updateMessageSchema = z.object({
  ...
  target_tiers: z
    .array(z.enum(["free", "basic", "pro", "premium"]))
    .optional()
    .nullable(),
...
export async function GET(
  ...
  try {
    ...
    const stats = {
      ...
      email_stats: {
        sent:
          message.beta_email_log?.filter(
            (e: { email_status: string }) => e.email_status === "sent"
          ).length || 0,
        failed:
          message.beta_email_log?.filter(
            (e: { email_status: string }) => e.email_status === "failed"
          ).length || 0,
        pending:
          message.beta_email_log?.filter(
            (e: { email_status: string }) => e.email_status === "pending"
          ).length || 0,
      },
      interaction_breakdown: message.beta_message_interactions?.reduce(
