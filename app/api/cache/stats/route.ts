...
export async function GET(request: NextRequest) {
  try {
      ...
      lines.forEach((line) => {
        if (line.includes(":")) {
          ...
          switch (trimmedKey) {
            ...
            case "expired_keys":
            case "evicted_keys":
              ;(redisInfo as any)[trimmedKey] = isNaN(Number(value))
                ? value
                : Number(value)
              break
            case "db0":
    ...
    const totalKeys = Object.values(patternStats).reduce(
      (sum, count) => sum + count,
      0
    )
    const hitMissRatio =
      redisInfo.keyspace_misses && redisInfo.keyspace_hits
    ...
    const response = {
      ...
      health: {
        ...
        memoryUsagePercent:
          redisInfo.used_memory && process.env.REDIS_MAX_MEMORY
            ? (Number(redisInfo.used_memory) /
                Number(process.env.REDIS_MAX_MEMORY)) *
              100
            : null,
      },
...
function generateRecommendations(
  ...
  if (appStats.hitRate < 0.5) {
    recommendations.push(
      "Low hit rate detected. Consider implementing cache warming strategies."
    )
  }
  ...
  if (appStats.similarityRate < 0.1 && appStats.similarityHits > 0) {
    recommendations.push(
      "Low similarity match rate. Consider adjusting similarity thresholds."
    )
  }
  ...
  if (redisInfo.evicted_keys && redisInfo.evicted_keys > 1000) {
    recommendations.push(
      "High key eviction rate. Consider increasing Redis memory limit."
    )
  }
  ...
  if (appStats.avgResponseTime > 100) {
    recommendations.push(
      "High average response time. Consider enabling compression or connection pooling."
    )
  }

  // Check pattern distribution
  const totalPatternKeys = Object.values(patternStats).reduce(
    (sum, count) => sum + count,
    0
  )
  if (patternStats.user_cache > totalPatternKeys * 0.7) {
    recommendations.push(
      "User cache dominates storage. Consider implementing user-specific TTLs."
    )
  }
  ...
  if (
    ...
    redisInfo.expired_keys > totalPatternKeys * 0.3
  ) {
    recommendations.push(
      "High number of expired keys. Consider running cache invalidation more frequently."
    )
  }
