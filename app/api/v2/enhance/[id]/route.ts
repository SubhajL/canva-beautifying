    ...
    try {
      ...
      const response = {
        ...
        processingTime: enhancement.completed_at
          ? new Date(enhancement.completed_at).getTime() -
            new Date(enhancement.created_at).getTime()
          : null,
        links: {
      ...
      logger.error(
        { err: error, requestId, enhancementId },
        "Failed to get enhancement"
      )
  ...
  async (request: NextRequest, { params }: RouteParams) => {
    ...
    try {
      ...
      if (
        ...
        throw apiErrors.VALIDATION_ERROR("Enhancement cannot be cancelled", {
          status: enhancement.status,
        })
      }
      ...
      if (enhancement.job_id) {
        ...
        try {
          ...
          logger.warn(
            { err: error, jobId: enhancement.job_id },
      ...
      if (
        ...
        error === apiErrors.VALIDATION_ERROR
      ) {
        throw error
      }
