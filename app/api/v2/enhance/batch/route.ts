        ...
        const results = await Promise.allSettled(
          files.map(async (file, index) => {
            try {
              ...
              const priority =
                fileSettings[index]?.priority ||
                globalSettings.priority ||
                "normal"
              ...
              logger.error(
                { err: error, fileName: file.name, batchId },
        ...
        return successResponse(
          {
            batchId,
            totalFiles: files.length,
            queuedFiles: successfulFiles.filter((f) => f.status === "queued")
              .length,
            ...
            links: {
              ...
              webhooks: globalSettings.webhookUrl
                ? `/api/v2/webhooks`
                : undefined,
            },
          },
