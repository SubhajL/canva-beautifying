...
export default function BatchUploadPage() {
  ...
  useEffect(() => {
    ...
    if (loading) {
      const timeout = setTimeout(() => {
        console.warn(
          "[BatchUploadPage] Loading timeout reached, auth might be stuck"
        )
      }, 5000)
  ...
  useEffect(() => {
    ...
    socket.on(
      "batch-complete",
      (data: { completedCount: number; totalCount: number }) => {
        setProcessing(false)
        toast({
      ...
      await new Promise((resolve) => {
        const checkUploads = setInterval(() => {
          const uploadingCount = files.filter(
            (f) => f.selected && f.status === "uploading"
          ).length
  ...
  const handleDownloadAll = async () => {
    const completedFiles = files.filter(
      (f) => f.status === "completed" && f.enhancedUrl
    )
    ...
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="space-y-4 text-center">
        ...
        <p className="text-lg text-muted-foreground">
          Upload multiple documents and enhance them all at once. Process up to{" "}
          {getBatchLimit()} files in a single batch.
        </p>
      </div>
      ...
      {!hasCredits() && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            You&apos;ve used all your monthly credits. Please upgrade your plan
            to continue enhancing documents.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          {/* Progress Tracker */}
          {files.length > 0 && <BatchProgressTracker files={files} />}
        </div>
      </div>
