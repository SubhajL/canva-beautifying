...
export default function BetaAnalyticsDashboard() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const uniqueActiveUsers = new Set(events?.map((e) => e.user_id) || [])
        .size
      ...
      const previousUniqueUsers = new Set(
        previousEvents?.map((e) => e.user_id) || []
      ).size
      const userGrowth =
        previousUniqueUsers > 0
          ? ((uniqueActiveUsers - previousUniqueUsers) / previousUniqueUsers) *
            100
          : 0
      ...
      const featureEvents =
        events?.filter((e) => e.event_type === "feature_usage") || []
      ...
      const ratings =
        feedback?.filter((f) => f.rating).map((f) => f.rating) || []
      const avgRating =
        ratings.length > 0
      ...
      const avgSessionDuration =
        validSessions.length > 0
          ? validSessions.reduce((acc, s) => acc + (s.duration || 0), 0) /
      ...
      const avgPageViews =
        validSessions.length > 0
          ? validSessions.reduce((acc, s) => acc + (s.pageViews || 0), 0) /
            validSessions.length
          : 0
      ...
      events?.forEach((event) => {
        if (userEngagementMap[event.user_id]) {
          ...
          if (
            !userEngagement.lastActive ||
            eventDate > userEngagement.lastActive
          ) {
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={Object.entries(
                          analyticsData?.feedbackStats.byType || {}
                        ).map(([type, count]) => ({
                          name: type,
                          value: count,
                        }))}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold">
                    {analyticsData?.feedbackStats.resolutionRate.toFixed(0) ||
                      0}
                    %
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Most visited pages by beta users
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {analyticsData?.sessionMetrics.topPages
                    .slice(0, 5)
                    .map((page, index) => (
                      <div
                        key={index}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {user.feedbackCount} feedback · Last active{" "}
                          {user.lastActive
