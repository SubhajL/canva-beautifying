...
import { Zap, TrendingUp, FileText, Clock, AlertCircle } from "lucide-react"
...
export function UsageStats() {
  const { tier, getRemainingCredits, tierDetails } = useSubscription()
  const _loading = false // No loading state in useSubscription hook

  const { stats, loading: statsLoading } = useUsageTracking()
  ...
  const isLowCredits =
    remainingCredits < tierDetails.features.monthlyCredits * 0.2
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Usage Statistics</CardTitle>
          <CardDescription>
            {tier.charAt(0).toUpperCase() + tier.slice(1)} Plan • Resets in{" "}
            {daysUntilReset} days
          </CardDescription>
        </CardHeader>
          ...
          {stats?.usageTrend && (
            <div className="border-t pt-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                />
                <span>
                  {Math.abs(stats.usageTrend)}%{" "}
                  {stats.usageTrend > 0 ? "increase" : "decrease"} from last
                  month
                </span>
              </div>
            </div>
