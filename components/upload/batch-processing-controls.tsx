...
export function BatchProcessingControls({
  ...
  const canStartProcessing =
    selectedFiles.length > 0 && !isProcessing && !disabled
  const canDownload = completedCount > 0
  const canGenerateReport =
    files.length > 0 && (completedCount > 0 || errorCount > 0)

  return (
    <Card>
      <CardHeader>
        <CardTitle>Batch Processing Controls</CardTitle>
        <CardDescription>Manage your batch enhancement process</CardDescription>
      </CardHeader>
        ...
        {isProcessing && (
          <div className="rounded-lg bg-blue-50 p-3 dark:bg-blue-950/20">
            <p className="text-sm text-blue-800 dark:text-blue-200">
              ...
              Processing will continue in the background, and you&apos;ll be
        ...
        {errorCount > 0 && (
          <div className="rounded-lg bg-red-50 p-3 dark:bg-red-950/20">
            <p className="text-sm text-red-800 dark:text-red-200">
              <strong>Note:</strong> {errorCount} file
              {errorCount > 1 ? "s" : ""} failed to process. Check the
              individual file status for details.
            </p>
          </div>
        )}
      </CardContent>
