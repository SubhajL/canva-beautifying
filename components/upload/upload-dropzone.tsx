...
export function UploadDropzone({
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      announce(
        `${newUploads.length} file${newUploads.length > 1 ? "s" : ""} selected for upload`,
        "polite"
      )
      ...
      try {
        ...
        announce(
          `Upload complete for ${newUploads.length} file${newUploads.length > 1 ? "s" : ""}`,
          "assertive"
        )
      } catch (error) {
      ...
      {!hasUploads || uploads.length < config.maxFiles ? (
        <div
          ...
          className={cn(
            ...
            (isUploading || uploads.length >= config.maxFiles) &&
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center space-y-4 text-center">
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              ...
              <span>•</span>
              <span>
                {uploads.length}/{config.maxFiles} files
              </span>
            </div>
          </div>
        </div>
