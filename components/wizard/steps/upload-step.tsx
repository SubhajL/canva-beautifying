    ...
    (fileRejections: any[]) => {
      if (fileRejections.length > 0) {
        ...
        if (errors.some((e: any) => e.code === "file-invalid-type")) {
          errorMessage =
            "File type not supported. Please upload PDF, PowerPoint, or Word documents."
        ...
        <div className="mx-auto max-w-md">
          ...
          {usage.percentageUsed >= 80 && usage.percentageUsed < 100 && (
            <Alert className="mt-4">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                You&apos;ve used {Math.round(usage.percentageUsed)}% of your
                monthly credits
              </AlertDescription>
            </Alert>
          )}
      ...
      {data.files.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start space-x-2 text-sm text-muted-foreground">
            ...
            <p>
              Your document will be securely processed. We do not store any
              personal information contained in your files.
            </p>
          </div>
        </div>
      )}
