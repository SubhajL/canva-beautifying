...
export function UpsellPrompt() {
  const { user } = useAuth()
  const { tier, canUpgrade, getRemainingCredits, tierDetails } =
    useSubscription()
  const [showDialog, setShowDialog] = useState(false)
  const [currentTrigger, setCurrentTrigger] = useState<UpsellTrigger | null>(
    null
  )
  ...
  const upsellTriggers: UpsellTrigger[] = [
    {
      ...
      description:
        "Upgrade now to continue enhancing your documents without interruption.",
      icon: <Zap className="h-8 w-8 text-amber-500" />,
      cta: "Upgrade to Continue",
      targetTier:
        tier === "free" ? "basic" : tier === "basic" ? "pro" : "premium",
    },
    {
      condition: () => {
      ...
      description:
        "You only have a few credits left. Upgrade for more monthly credits and better features.",
    ...
    },
    {
      ...
      description:
        "Based on your current usage pattern, you might benefit from a higher tier with more credits.",
    ...
    },
    {
      ...
      description:
        "Get access to advanced AI models, batch processing, and priority support.",
  ...
  const checkUpsellTriggers = () => {
    ...
    const allDismissedToday =
      dismissed.size >= upsellTriggers.length &&
  ...
  const getTargetTierDetails = () => {
    ...
    return SUBSCRIPTION_TIERS[
      currentTrigger.targetTier.toUpperCase() as keyof typeof SUBSCRIPTION_TIERS
    ]
  }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {targetTier && (
          <Card className="mt-4">
            <CardHeader className="pb-4">
              ...
              <CardDescription>${targetTier.price}/month</CardDescription>
            </CardHeader>
        ...
        <div className="mt-4 flex gap-2">
          <Link href="/app/settings/billing" className="flex-1">
            <Button className="w-full">{currentTrigger.cta}</Button>
          </Link>
...
export function InlineUpsellCard({
  ...
  const nextTierDetails =
    SUBSCRIPTION_TIERS[
      nextTier.toUpperCase() as keyof typeof SUBSCRIPTION_TIERS
    ]
  ...
  return (
    <Card className={`border-primary/20 bg-primary/5 ${className}`}>
      <CardHeader>
        ...
        <CardDescription>
          Upgrade to {nextTierDetails.name} for{" "}
          {nextTierDetails.features.monthlyCredits} monthly credits
        </CardDescription>
      </CardHeader>
      <CardContent>
