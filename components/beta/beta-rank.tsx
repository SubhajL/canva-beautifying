...
export function BetaRank({ userId }: BetaRankProps) {
  ...
  useEffect(() => {
    const fetchRankData = async () => {
      try {
        ...
        const betaJoinedAt = profile?.beta_joined_at
          ? new Date(profile.beta_joined_at)
          : new Date()
        const daysInBeta = Math.floor(
          (new Date().getTime() - betaJoinedAt.getTime()) /
            (1000 * 60 * 60 * 24)
        )
        ...
        const feedbackStats = feedback?.reduce(
          (acc, item) => {
            acc.total++
        ...
        const nextLevel = RANK_LEVELS.find(
        ...
        const earnedBadges = BADGES.map((badge) => {
          ...
          if (
            badge.threshold.feedbackCount &&
            feedbackStats.total >= badge.threshold.feedbackCount
          ) {
            earned = true
          }
          if (
            badge.threshold.bugCount &&
            feedbackStats.bugs >= badge.threshold.bugCount
          ) {
            earned = true
          }
          if (
            badge.threshold.featureCount &&
            feedbackStats.features >= badge.threshold.featureCount
          ) {
            earned = true
          }
          if (
            badge.threshold.enhancementCount &&
            (enhancementCount || 0) >= badge.threshold.enhancementCount
          ) {
            earned = true
          }
          if (
            badge.threshold.daysInBeta &&
            daysInBeta >= badge.threshold.daysInBeta
          ) {
            earned = true
          }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Level up by contributing to BeautifyAI
          </CardDescription>
        </CardHeader>
  ...
  const CurrentRankIcon =
    RANK_LEVELS.find((l) => l.level === rankData.level)?.icon || Star
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {rankData.level < 5 && (
          <div className="space-y-2">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {Math.round(rankData.nextLevelScore - rankData.score)} points to
              next level
            </p>
          </div>
        )}
        ...
        {/* Available Badges */}
        <div className="space-y-2">
          <h4 className="text-sm font-medium">Available Badges</h4>
          <div className="flex flex-wrap gap-2">
            {rankData.badges
              .filter((b) => !b.earned)
              .map((badge) => (
