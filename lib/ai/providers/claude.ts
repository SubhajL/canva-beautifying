...
export class ClaudeProvider extends BaseAIProvider {
  ...
  constructor(
    ...
    modelVersion: "claude-3.5-sonnet" | "claude-4-sonnet" = "claude-3.5-sonnet"
  ) {
    super(config)
    ...
    try {
      ...
      const response = await this.retryWithBackoff(async () => {
        const res = await fetch(`${this.baseUrl}/messages`, {
          ...
          body: JSON.stringify({
            model:
              this.modelVersion === "claude-3.5-sonnet"
            ...
            system:
              "You are a professional document design analyzer. Always respond with valid JSON that matches the DocumentAnalysis interface structure.",
          }),
      ...
      const response = await this.retryWithBackoff(async () => {
        const res = await fetch(`${this.baseUrl}/messages`, {
          ...
          body: JSON.stringify({
            ...
            system:
              "You are an expert in creating detailed prompts for AI image generation that enhance document designs while maintaining their core purpose.",
          }),
          signal: AbortSignal.timeout(this.config.timeout || 30000),
        })
