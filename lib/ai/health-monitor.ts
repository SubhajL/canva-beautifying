...
export class AIHealthMonitor {
  ...
  private healthHistory: Map<
    string,
    Array<{ timestamp: number; health: number }>
  > = new Map()
  ...
  async checkProviderHealth(provider: string): Promise<HealthMetrics> {
    ...
    try {
      ...
      const currentMetrics =
        this.healthMetrics.get(provider) || this.createDefaultMetrics(provider)
    ...
    providers.forEach((provider) => {
      ...
      if (metrics) {
        const score = metrics.healthy
          ? (1 - metrics.errorRate) * (1 - Math.min(metrics.latency / 5000, 1))
          : 0.05
