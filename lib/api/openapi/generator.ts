...
export function generateOpenAPISpec(): OpenAPIDocument {
  ...
  for (const [path, pathRoutes] of routesByPath) {
    ...
    for (const route of pathRoutes) {
      const method =
        route.metadata.method.toLowerCase() as keyof OpenAPIPathItem
  ...
  if (route.request) {
    ...
    if (route.request.body) {
      const requestBody: OpenAPIRequestBody = {
        ...
        required:
          route.request.body.required !== undefined &&
          typeof route.request.body.required !== "boolean"
            ? true
      ...
      if (route.request.body.content) {
        ...
        for (const [contentType, contentDef] of Object.entries(
          ...
          const schema =
            contentDef.schema.type ||
            contentDef.schema.oneOf ||
            contentDef.schema.$ref
          ...
          if (contentDef.examples) {
            requestBody.content[contentType].examples = Object.entries(
              contentDef.examples
            ).reduce(
              (acc, [name, value]) => ({
                ...acc,
        ...
        const schema = route.request.body.type
          ? route.request.body
          : zodToOpenAPISchema(route.request.body)
    ...
    for (const [statusCode, responseDef] of Object.entries(route.responses)) {
      ...
      if (responseDef.content) {
        ...
        for (const [contentType, contentDef] of Object.entries(
          ...
          // Add examples if provided
          if (contentDef.examples) {
            responses[statusCode].content![contentType].examples =
              Object.entries(contentDef.examples).reduce(
                (acc, [name, value]) => ({
                  ...acc,
                  [name]: { value },
                }),
        ...
        const schema = responseDef.schema.type
          ? responseDef.schema
          : zodToOpenAPISchema(responseDef.schema)
 ...
 */
export function validateOpenAPISpec(spec: OpenAPIDocument): {
  valid: boolean
  errors?: string[]
} {
  const errors: string[] = []
    ...
    for (const operation of operations) {
      if (operation in pathItem) {
        ...
        if (op && (!op.responses || Object.keys(op.responses).length === 0)) {
          errors.push(
            `Operation ${operation.toUpperCase()} ${path} must have at least one response`
          )
        }
      }
    }
