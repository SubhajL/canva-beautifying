...
export class BeautifyAIClient {
  ...
  async getHistory(
    query?: HistoryQuery
  ): Promise<PaginatedResponse<EnhancementStatus>> {
...
export function createClient(
  config?: Partial<BeautifyAIConfig>
): BeautifyAIClient {
  const token =
    config?.token ||
    process.env.BEAUTIFYAI_API_TOKEN ||
    process.env.NEXT_PUBLIC_BEAUTIFYAI_API_TOKEN
  ...
  return new BeautifyAIClient({
    ...
    baseUrl:
      config?.baseUrl ||
      process.env.BEAUTIFYAI_API_URL ||
      process.env.NEXT_PUBLIC_BEAUTIFYAI_API_URL,
    ...config,
  })
}
