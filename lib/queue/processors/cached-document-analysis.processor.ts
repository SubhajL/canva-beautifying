...
export class CachedDocumentAnalysisProcessor {
  ...
  private async generateDocumentHash(
  ...
  private calculateComplexity(analysis: DocumentAnalysis): DocumentComplexity {
    const factors = {
      elementCount:
        (analysis.layout.issues.length + analysis.colors.palette.length) / 10,
    ...
    return {
      level:
        avgComplexity < 0.3 ? "low" : avgComplexity < 0.6 ? "medium" : "high",
      factors,
    }
  }
