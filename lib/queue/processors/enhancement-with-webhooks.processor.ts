...
const worker = new Worker<EnhancementJobData, JobResult>(
    ...
    async (job: Job<EnhancementJobData>) =>
      withJobTrace(job.data as any, "queue.enhancement", async () => {
      ...
      try {
        ...
        await webhookManager.triggerWebhooks(userId, "enhancement.started", {
        ...
        await webhookManager.triggerWebhooks(userId, "enhancement.progress", {
          enhancementId: job.id!,
          documentId,
          status: "processing",
          progress: 5,
...
const { url: enhancedFileUrl } = await uploadBufferToKey({
          buffer: enhancedDocument.buffer,
          key: enhancedFileKey,
...
const { url: thumbnailUrl } = await uploadBufferToKey({
          buffer: enhancedDocument.thumbnail,
          key: thumbnailKey,
        ...
        return { success: true, enhancementId: enhancement.id } as any

        const processingTime = Date.now() - startTime

        // Trigger webhook: enhancement.completed
        await webhookManager.triggerWebhooks(userId, "enhancement.completed", {
          enhancementId: job.id!,
          documentId,
          status: "completed",
          progress: 100,
        ...
        await webhookManager.triggerWebhooks(userId, "enhancement.failed", {
          enhancementId: job.id!,
          documentId,
          status: "failed",
          error: {
