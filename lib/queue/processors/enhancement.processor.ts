...
import {
  pipelineTracer,
  activateJobTraceContext,
} from "@/lib/observability/enhancement-pipeline"
...
export const createEnhancementWorker = () => {
  const worker = new Worker<EnhancementJobData, JobResult>(
    ...
    async (job: Job<EnhancementJobData>) => {
      // Restore trace context and wrap entire job execution
...
const { url: enhancedFileUrl } = await uploadBufferToKey({
          ...
          if (enhanceError) {
            throw new Error(
              `Failed to save enhancement: ${enhanceError.message}`
            )
          }
