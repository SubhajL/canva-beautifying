...
export const createEnhancementWorker = () => {
  const worker = new Worker<EnhancementJobData, JobResult>(
    ...
    async (job: Job<EnhancementJobData>) => {
      const restoredContext = activateJobTraceContext(job.data);
      return context.with(restoredContext, async () => {
        ...
        try {
          ...
          const enhancedFileKey = `enhanced/${userId}/${documentId}/${Date.now()}.${
            document.file_type
          }`;
          const enhancedFileUrl = await uploadToR2(
            enhancedDocument.buffer,
            enhancedFileKey
          );
