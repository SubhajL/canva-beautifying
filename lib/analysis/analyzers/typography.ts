...
export class TypographyAnalyzer {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (edges[idx] === 255 && !visited.has(idx)) {
          ...
          if (
            region.width > this.MIN_TEXT_BLOCK_SIZE &&
  ...
  private estimateLineHeight(
    ...
    for (let y = 0; y < projection.length; y++) {
      if (projection[y] < region.width * 0.1) {
        // Less than 10% filled
        if (!inGap) {
          inGap = true
          gapStart = y
  ...
  private estimateFontMetrics(
    textBlocks: TextBlock[]
  ): TypographyMetrics["fonts"] {
  ...
  private calculateReadability(
    ...
    const lineHeightRatios = textBlocks.map(
      (block) => block.lineHeight / block.fontSize
    )
    const avgLineHeightRatio =
      lineHeightRatios.reduce((a, b) => a + b) / lineHeightRatios.length

    // Optimal line height ratio is 1.4-1.6
    const lineHeightScore = this.scoreInRange(
      avgLineHeightRatio,
      1.4,
      1.6,
      1.2,
    ...
    const avgDensity =
      textBlocks.map((b) => b.characterDensity).reduce((a, b) => a + b) /
      textBlocks.length
