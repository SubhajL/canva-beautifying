...
export class AgeAppropriatenessAnalyzer {
  async analyze(context: DocumentContext): Promise<AgeAppropriateness> {
    ...
    const detectedAge = this.determineTargetAge(
      complexity,
      visualStyle,
      contentMaturity,
      userPreferences?.targetAudience
    )
  ...
  private analyzeContentMaturity(
    imageData: ImageData,
    ...
    const maturity =
      textDensity * 0.4 + formalElements * 0.3 + professionalIndicators * 0.3
  ...
  private calculateConfidence(
    ...
    const variance =
      factors.reduce((sum, f) => sum + Math.pow(f - mean, 2), 0) /
      factors.length
  ...
  private detectEdgeOrientations(_: ImageData): {
    curved: number
    straight: number
  } {
