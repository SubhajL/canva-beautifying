...
export class LayoutAnalyzer {
  ...
  private detectAlignment(imageData: ImageData): LayoutMetrics["alignment"] {
    ...
    const horizontalCenters = contentBlocks.map(
      (block) => block.x + block.width / 2
    )
    const horizontalAlignment = this.determineAlignment(
      horizontalCenters,
      width
    )

    // Analyze vertical alignment
    const verticalCenters = contentBlocks.map(
      (block) => block.y + block.height / 2
    )
    const verticalAlignment = this.determineVerticalAlignment(
      verticalCenters,
      height
    )
  ...
  private detectGrid(imageData: ImageData): LayoutMetrics["grid"] {
    ...
    const gridScore =
      (horizontalSpacing.consistency + verticalSpacing.consistency) / 2
  ...
  private measureMargins(imageData: ImageData): LayoutMetrics["margins"] {
    ...
    const variance =
      marginValues.reduce((sum, m) => sum + Math.pow(m - avgMargin, 2), 0) / 4
  ...
  private findContentBounds(imageData: ImageData): {
    minX: number
    minY: number
    maxX: number
    maxY: number
  ...
  private calculateAlignmentConsistency(
