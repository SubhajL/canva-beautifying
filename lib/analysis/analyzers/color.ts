...
export class ColorAnalyzer {
  ...
  private buildPalette(colors: Map<string, number>): ColorMetrics["palette"] {
    ...
    categorizedColors.forEach((color, index) => {
      ...
      } else if (color.hsl.s > 50) {
        // High saturation colors as accents
  ...
  private checkAccessibility(
    ...
    const hasProblematicReds = this.hasProblematicColorBlindCombos(
      colorArray,
      "protanopia"
    )
    const hasProblematicGreens = this.hasProblematicColorBlindCombos(
      colorArray,
      "deuteranopia"
    )
    const hasProblematicBlues = this.hasProblematicColorBlindCombos(
      colorArray,
      "tritanopia"
    )
  ...
  private calculateCustomHarmonyScore(colors: HSL[]): number {
    ...
    const satVariance =
      saturations.reduce((sum, s) => sum + Math.pow(s - avgSat, 2), 0) /
      saturations.length
    const lightVariance =
      lightnesses.reduce((sum, l) => sum + Math.pow(l - avgLight, 2), 0) /
      lightnesses.length
  ...
  private hasProblematicColorBlindCombos(
    colors: RGB[],
    ...
    for (let i = 0; i < colors.length; i++) {
      for (let j = i + 1; j < colors.length; j++) {
        ...
        const simulatedDiff = this.colorDistance(
          simulatedColors[i],
          simulatedColors[j]
        )
