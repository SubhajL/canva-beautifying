...
export class CanvaExporter extends BaseExporter {
  ...
  async export(request: ExportRequest): Promise<ExportResult> {
    try {
      const { result: canvaData, time } = await this.measureProcessingTime(
        async () => {
          ...
          if (request.metadata?.enhancements) {
            this.addEnhancementElements(
              canvaFormat,
              request.metadata.enhancements as Record<string, unknown>
            )
          }
  ...
  private inferDocumentType(
    ...
    if (Math.abs(aspectRatio - 1.778) < 0.1) {
      // 16:9
      return "PRESENTATION"
    } else if (Math.abs(aspectRatio - 0.707) < 0.1) {
      // A4 portrait
      return "DOCUMENT"
    }
  ...
  private addEnhancementElements(
    ...
    if (enhancements.decorativeElements) {
      const elements = enhancements.decorativeElements as Array<
