...
export class PDFAnalyzer {
  ...
  async analyzePDFStructure(file: File): Promise<PDFAnalysis> {
    ...
    try {
      ...
      const corruptionReport = await this.detectPDFCorruption(
        Buffer.from(buffer)
      )
      if (corruptionReport.isCorrupted) {
        analysis.errors.push(
          ...corruptionReport.indicators.map((i) => i.message)
        )
      ...
      try {
        ...
        if (
          corruptionReport.isCorrupted ||
          corruptionReport.indicators.length > 0
        ) {
          analysis.warnings.push(corruptionReport.recommendation)
        }
      } catch {
    ...
    analysis.warnings = analysis.warnings.map((warn) =>
      sanitizeErrorMessage(warn)
    )
  ...
  async detectPDFCorruption(buffer: Buffer): Promise<CorruptionReport> {
    ...
    try {
      ...
      if (
        objMatches.length > 0 &&
        Math.abs(objMatches.length - endobjMatches.length) >= 2
      ) {
        indicators.push({
          type: "structure",
          severity: "medium",
      ...
      if (nullByteRatio > 0.7) {
        // Increased threshold
        indicators.push({
          type: "encoding",
          severity: "high",
          message:
            "Excessive null bytes detected - possible encoding corruption",
        })
      }
    ...
    const canBeProcessed =
      !isCorrupted || indicators.every((i) => i.type !== "structure")
