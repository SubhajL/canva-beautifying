...
export function detectDocumentType(text: string, metadata: any): string {
  ...
  const educationalCount = educationalPatterns.filter((p) =>
    lowerText.includes(p)
  ).length
  const presentationCount = presentationPatterns.filter((p) =>
    lowerText.includes(p)
  ).length
  const marketingCount = marketingPatterns.filter((p) =>
    lowerText.includes(p)
  ).length
