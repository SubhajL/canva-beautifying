...
const getCSRFSecret = (): string => {
  ...
  if (
    ...
    secret.startsWith("default-csrf-secret")
  ) {
    throw new Error(
...
export async function csrfProtection(
  ...
  const publicEndpoints = ["/api/auth", "/api/webhook", "/api/health"]
...
export function validateCSRFSecret(secret: string): {
  valid: boolean
  error?: string
} {
  if (!secret) {
  ...
  if (secret.startsWith("default-csrf-secret")) {
    return {
      valid: false,
      error: "Default CSRF secret detected - please change it",
    }
  }
