...
export class AuthRateLimiter {
  ...
  async checkPasswordResetAttempt(
    ...
    return {
      allowed: true,
      remainingAttempts:
        this.passwordResetLimiter.getRemainingAttempts(identifier),
    }
  }
  ...
  async checkSignupAttempt(
    ...
    if (!this.signupLimiter.isAllowed(email)) {
      return {
        allowed: false,
        reason:
          "This email has been used for signup recently. Please try again later.",
      }
    }
  ...
  async checkVerificationAttempt(identifier: string): Promise<{
    allowed: boolean
    remainingAttempts?: number
    reason?: string
  }> {
    ...
    return {
      allowed: true,
      remainingAttempts:
        this.verificationLimiter.getRemainingAttempts(identifier),
    }
  }
  ...
  private checkForSuspiciousActivity(
    ...
    const uniqueUserAgents = new Set(
      attempts.map((a) => a.userAgent).filter(Boolean)
    )
    ...
    if (failedCount >= 5) {
      this.blockIdentifier(
        identifier,
        Math.min(failedCount * 5 * 60 * 1000, 60 * 60 * 1000)
      )
    }
  }
