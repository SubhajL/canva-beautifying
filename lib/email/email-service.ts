...
import { EmailDeliveryResult, EmailTemplateType, EmailRecipient } from "./types"
...
export class EmailService {
  ...
  static async sendWelcomeEmail(
    ...
    if (!resend) {
      console.log(
        "Email service not configured. Would send welcome email to:",
        data.userEmail
      )
      return {
        id: `dev-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  static async sendPasswordResetEmail(data: {
    ...
    if (!resend) {
      console.log(
        "Email service not configured. Would send password reset email to:",
        data.userEmail
      )
      return {
        id: `dev-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  static async sendSubscriptionCreatedEmail(
    ...
    if (!resend) {
      console.log(
        "Email service not configured. Would send subscription created email to:",
        data.userEmail
      )
      return {
        id: `dev-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        ...
        batch.map(async (recipient) => {
          try {
            ...
            switch (emailType) {
              ...
              default:
                throw new Error(
                  `Bulk email not implemented for type: ${emailType}`
                )
            }
