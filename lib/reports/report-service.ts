...
export class ReportService {
  ...
  private async getDocumentInfo(
    documentId: string,
    userId: string
  ): Promise<EnhancementReport["documentInfo"]> {
    const supabase = await createClient()
  ...
  private inferDocumentType(
    metadata: any
  ): EnhancementReport["documentInfo"]["documentType"] {
  ...
  async applyCustomization(
    ...
    if (customization.detailLevel === "summary") {
      ...
      customizedReport.enhancements.applied =
        customizedReport.enhancements.applied.slice(0, 3)
    ...
    if (!customization.includeTechnicalDetails) {
      ...
      customizedReport.enhancements.applied =
        customizedReport.enhancements.applied.map((e) => ({
          ...e,
    ...
    if (customization.focusAreas && customization.focusAreas.length > 0) {
      ...
      customizedReport.enhancements.applied =
        customizedReport.enhancements.applied.filter((e) =>
          customization.focusAreas!.includes(e.category)
        )
    }
