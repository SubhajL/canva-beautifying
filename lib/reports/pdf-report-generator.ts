...
export class PDFReportGenerator {
  ...
  private async createCoverPage(
    ...
    const scoreColor =
      improvement > 20 ? this.colors.success : this.colors.secondary
  ...
  private async createSummaryPage(
    ...
    const metrics = [
      ...
      {
        label: "Enhancements Applied",
        value: report.enhancements.totalCount.toString(),
      },
      {
        label: "Processing Time",
        value: `${report.metadata.processingTime}ms`,
      },
      {
        label: "Engagement Prediction",
        value: `${report.engagement.predictedScore}%`,
      },
    ]
    ...
    page.drawText(
      `Engagement Likelihood: ${report.engagement.audienceImpact.engagementLikelihood.toUpperCase()}`,
      {
        x: 50,
        y: yPosition,
        size: 12,
    ...
    report.engagement.audienceImpact.keyImprovements
      .slice(0, 3)
      .forEach((improvement) => {
        page.drawText(`• ${improvement}`, {
          x: 60,
  ...
  private async createComparisonPage(
    ...
    const categories = [
      {
        name: "Overall Score",
        before: report.comparison.before.overallScore,
        after: report.comparison.after.overallScore,
      },
      {
        name: "Visual Appeal",
    ...
    categories.forEach((cat) => {
      ...
      page.drawText(cat.before.toString(), {
        x: 250,
        y: yPosition,
        size: 11,
        font: helvetica,
      ...
      page.drawText(cat.after.toString(), {
        x: 350,
        y: yPosition,
        size: 11,
        font: helvetica,
  ...
  private async createEnhancementsPage(
    ...
    categories.forEach(([category, count]) => {
      if (count > 0) {
        page.drawText(
          `${category.charAt(0).toUpperCase() + category.slice(1)}: ${count} enhancements`,
          {
            x: 50,
            y: yPosition,
            size: 14,
