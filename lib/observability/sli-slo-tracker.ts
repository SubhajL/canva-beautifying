...
export class SLISLOTracker extends EventEmitter {
  ...
  public async getSLOStatus(sloId: string): Promise<SLOStatus | null> {
    ...
    return traceAsync("slo.get_status", async () => {
      ...
      const prediction = await this.predictViolation(
        slo,
        currentValue,
        burnRate
      )
  ...
  private async predictViolation(
    ...
    const hoursUntilViolation = Math.abs(
      (errorBudgetRemaining / (avgBurnRate - 1)) * 24
    )

    // Confidence based on consistency of burn rates
    const burnRateConsistency =
      1 -
      Math.abs(burnRate.short - burnRate.long) /
        Math.max(burnRate.short, burnRate.long)
  ...
  private async checkAlertingConditions(status: SLOStatus) {
    ...
    const shouldAlertShort =
      status.burnRate.short >= slo.alerting.burnRate.short.threshold
    const shouldAlertLong =
      status.burnRate.long >= slo.alerting.burnRate.long.threshold
    ...
    if (
      status.prediction &&
      status.prediction.willViolateIn < 24 &&
      status.prediction.confidence > 80
    ) {
      this.emit("slo:alert", {
        severity: "warning",
        slo: slo.id,
  ...
  private getCounterBucketKey(
    ...
    const bucket = Math.floor(
      timestamp / (this.getWindowMilliseconds(window) / 10)
    )
    const labelStr = labels
      ? `:${Object.entries(labels)
          .map(([k, v]) => `${k}=${v}`)
