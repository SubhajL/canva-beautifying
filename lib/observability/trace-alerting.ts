...
export class TraceAlertingEngine extends EventEmitter {
  ...
  private async evaluateRule(
    ...
    const results = await Promise.all(
      rule.conditions.map((condition) =>
        this.evaluateCondition(condition, span, traceId)
      )
    )
  ...
  private async evaluateSpanCount(
    ...
    for (const span of spans) {
      if (condition.attribute) {
        ...
        if (
          value !== undefined &&
          this.compareAttribute(value, condition.attribute)
        ) {
          count++
        }
      } else {
  ...
  private determineSeverity(rule: AlertRule): Alert["severity"] {
    ...
    if (
      rule.id.includes("critical") ||
      rule.name.toLowerCase().includes("critical")
    ) {
      return "critical"
    } else if (
      rule.id.includes("error") ||
      rule.name.toLowerCase().includes("error")
    ) {
      return "error"
    } else if (
      rule.id.includes("warning") ||
      rule.name.toLowerCase().includes("warning")
    ) {
      return "warning"
    }
    return "info"
  ...
  private async executeSlackAction(action: AlertAction, alert: Alert) {
    ...
    const webhookUrl =
      action.config.webhook_url || process.env.SLACK_WEBHOOK_URL
    const color = {
      info: "#36a64f",
      warning: "#ff9800",
      error: "#f44336",
  ...
  private cleanupOldTraces() {
    ...
    for (const [traceId, spans] of this.spanBuffer) {
      if (spans.length > 0) {
        ...
        const endTime =
          latestSpan.endTime[0] * 1000 + latestSpan.endTime[1] / 1e6
