...
const pipelineMetrics = {
  stageStarted:
    metrics.register.getSingleMetric("enhancement_stage_started") ||
  ...
  stageCompleted:
    metrics.register.getSingleMetric("enhancement_stage_completed") ||
    new (require("prom-client").Counter)({
  ...
  stageDuration:
    metrics.register.getSingleMetric("enhancement_stage_duration") ||
  ...
  pipelineTotal:
    metrics.register.getSingleMetric("enhancement_pipeline_total") ||
    new (require("prom-client").Histogram)({
...
/**
 * Activate OpenTelemetry trace context from job data
 *
 * Extracts and restores trace context from BullMQ job data containing
 * W3C trace context headers (traceparent/tracestate), enabling distributed
 * tracing across queue boundaries.
 *
 * Returns the restored context which should be used with context.with()
 * to wrap the actual job execution logic.
 *
 * @param jobData - BullMQ job data containing optional traceContext field
 * @returns Restored OpenTelemetry context, or active context if no trace data
 *
 * @example
 * // In worker processor
 * const restoredCtx = activateJobTraceContext(job.data)
 * return context.with(restoredCtx, async () => {
 *   // Job execution logic here
 *   return await processJob(job)
 * })
 */
export function activateJobTraceContext(
  jobData: any
): ReturnType<typeof context.active> {
  // Check if job data contains trace context
  ...
  try {
    ...
    const restoredContext = restoreFromCarrier(jobData.traceContext)

    logger.debug(
      {
        traceContext: jobData.traceContext,
        jobId: jobData.enhancementId || jobData.documentId,
      },
      "Trace context restored from job data"
    )
    ...
    logger.warn(
      {
        err: error as Error,
        jobData: jobData.enhancementId || jobData.documentId,
      },
      "Failed to activate trace context from job data"
    )
