...
export class LogShipper extends EventEmitter {
  ...
  private isRateLimitError(error: any): boolean {
    return (
      this.categorizeError(error, {} as LogDestination) ===
  ...
  private shouldCountAsCircuitBreakerFailure(category: ErrorCategory): boolean {
    ...
    return (
      category !== ErrorCategory.RATE_LIMIT && category !== ErrorCategory.CLIENT
    )
  }
  ...
  private async processBatches(): Promise<void> {
    ...
    for (const [destName, batches] of this.batchQueue) {
      ...
      const destination = this.config.destinations.find(
        (d) => d.name === destName
      )
  ...
  private async processDestinationBatches(
    ...
    const unprocessedBatches = remainingBatches.filter(
      (b) => !batches.includes(b)
    )
    // Add failed batches back to queue
    this.batchQueue.set(destination.name, [
      ...unprocessedBatches,
      ...failedBatches,
    ])
  ...
  private async shipBatch(
    ...
    switch (destination.type) {
      case "elasticsearch":
        await this.shipToElasticsearch(
          destination.config as ElasticsearchConfig,
          logs
        )
        break
      case "cloudwatch":
        await this.shipToCloudWatch(
          destination.config as CloudWatchConfig,
          logs
        )
        break
      case "datadog":
      ...
      default:
        throw createSafeError(
          `Unknown destination type: ${destination.type}`,
          "UNKNOWN_DESTINATION"
        )
    }
  }

  private async shipToElasticsearch(
    ...
    const body = logs.flatMap((log) => [
      {
        index: {
          _index: `${config.index}-${new Date().toISOString().slice(0, 10)}`,
        },
      },
      {
  ...
  private async shipToCloudWatch(
