...
export class AIPerformanceTracker {
  ...
  public async trackModelPerformance(performance: AIModelPerformance) {
    await traceAsync("ai_performance.track", async () => {
      ...
      await this.redis.zadd(`ai:perf:raw:${key}`, performance.timestamp, value)
  ...
  private async updateRealtimeMetrics(performance: AIModelPerformance) {
    ...
    if (performance.tokensUsed) {
      ...
      pipeline.hincrby(
        statsKey,
        "tokens:completion",
        performance.tokensUsed.completion
      )
    }
  ...
  private categorizeError(error: string): string {
    ...
    if (error.includes("token") || error.includes("context"))
      return "token_limit"
    ...
    const allStats = await Promise.all(
      keys.map(async (key) => {
        ...
        return latestData.length > 0
          ? (JSON.parse(latestData[0]) as AIModelStats)
          : null
      })
    )
  ...
  public async recommendModel(
    ...
    for (const stats of allStats) {
      ...
      const providerScore = criteria.preferredProviders?.includes(
        stats.provider
      )
        ? 10
        : 0
      ...
      if (
        criteria.maxLatency &&
        stats.performance.avgDuration > criteria.maxLatency
      )
        continue
      if (
        criteria.maxCostPerRequest &&
        stats.cost.avgPerRequest > criteria.maxCostPerRequest
      )
        continue
      if (
        criteria.minSuccessRate &&
        stats.requests.successRate < criteria.minSuccessRate
      )
        continue
      ...
      const recentErrors = Object.values(stats.errors).reduce(
        (sum, count) => sum + count,
        0
      )
      const errorScore = Math.max(
        0,
        10 - (recentErrors / stats.requests.total) * 100
      )
      score += errorScore

      const reasons: string[] = []
      if (stats.requests.successRate >= 99)
        reasons.push("Very high reliability")
  ...
  > {
    ...
    for (const key in results) {
      if (results[key].requestCount > 0) {
        results[key].avgCostPerRequest =
          results[key].totalCost / results[key].requestCount
        results[key].avgTokensPerRequest =
          results[key].totalTokens / results[key].requestCount
      }
    }
  ...
  private async aggregateStats(period: "1h" | "24h" | "7d" | "30d") {
    try {
      ...
      for (const { model, provider } of models) {
        ...
        if (stats) {
          const key = `ai:aggregate:${model}:${provider}:${period}`
          await this.redis.zadd(key, stats.timestamp, JSON.stringify(stats))
  ...
  private async getActiveModels(): Promise<
  ...
  private async computeModelStats(
    ...
    return {
      ...
      requests: {
        ...
        failed,
        successRate:
          performances.length > 0 ? successful / performances.length : 0,
      },
      performance: {
        avgDuration:
      ...
      tokens: {
        total: totalTokens,
        avgPerRequest:
          performances.length > 0 ? totalTokens / performances.length : 0,
        promptAvg:
          performances.length > 0 ? totalPromptTokens / performances.length : 0,
        completionAvg:
          performances.length > 0
            ? totalCompletionTokens / performances.length
            : 0,
      },
      cost: {
        total: totalCost,
        avgPerRequest:
          performances.length > 0 ? totalCost / performances.length : 0,
...
export function initializeAIPerformanceTracker(
  redis: Redis
