...
export class PerformanceMonitor {
  ...
  private async updateAggregates(metric: PerformanceMetric, pipeline: any) {
    const periods: Array<"1m" | "5m" | "15m" | "1h" | "24h"> = [
      "1m",
      "5m",
      "15m",
      "1h",
    ...
    for (const period of periods) {
      const bucketKey = this.getAggregateBucketKey(
        metric.name,
        period,
        metric.timestamp
      )
      ...
      if (locked) {
        setTimeout(
          () =>
            this.computeAggregate(metric.name, period, bucketKey, aggregateKey),
          1000
        )
      }
    }
  ...
  private getAggregateBucketKey(
    metricName: string,
    period: string,
    timestamp: number
  ): string {
    const bucket = Math.floor(
      timestamp / (this.getPeriodSeconds(period) * 1000)
    )
