  ...
  private detectBottlenecks(usage: ResourceUsage): PerformanceBottleneck[] {
    ...
    if (usage.cpu.usage >= this.thresholds.cpu.critical) {
      bottlenecks.push({
        ...
        recommendation:
          "CPU usage is critically high. Consider scaling horizontally or optimizing CPU-intensive operations.",
        timestamp: Date.now(),
      })
    ...
    if (usage.memory.percent >= this.thresholds.memory.critical) {
      bottlenecks.push({
        ...
        recommendation:
          "Memory usage is critically high. Check for memory leaks or increase available memory.",
        timestamp: Date.now(),
      })
    ...
    if (usage.eventLoop.lag >= this.thresholds.eventLoopLag.critical) {
      bottlenecks.push({
        ...
        recommendation:
          "Event loop lag is critically high. Offload synchronous operations to worker threads.",
        timestamp: Date.now(),
      })
      ...
      bottlenecks.push({
        ...
        recommendation:
          "Event loop lag detected. Review synchronous operations.",
        timestamp: Date.now(),
      })
    }
        ...
        if (currentValue < threshold) {
          ...
          if (stepsToThreshold > 0 && stepsToThreshold < 100) {
            const timeToThreshold =
              stepsToThreshold * (duration === "1h" ? 5 : 60) // minutes
            prediction = {
