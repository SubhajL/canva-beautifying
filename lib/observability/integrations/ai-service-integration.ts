...
export class MonitoredAIProvider extends BaseAIProvider {
  ...
  async generateAssets(enhancement: any, options?: any) {
    return monitorAIOperation(
      ...
      async () => {
        ...
        try {
          const result = await this.provider.generateAssets(
            enhancement,
            options
          )
          const duration = Date.now() - startTime
          ...
          return {
            ...result,
            cost: this.calculateAssetGenerationCost(
              (result as any).assets?.length || 0
            ),
          }
        } catch (error) {
  ...
  private handleRateLimit(error: Error) {
    ...
    if (match) {
      ...
      logAIRateLimit(this.model, this.providerName, limit, remaining)
    }
  }
}
...
export class MonitoredModelSelector {
  static selectModel(
    ...
    // Log the selection
    logModelSelection(preferredModel || "auto", result.model, result.reason, {
      userTier,
    ...
    const duration = Date.now() - startTime
    logger.logPerformanceMetric("ai.model_selection.duration", duration, "ms", {
      user_tier: userTier,
...
export function createMonitoredAIService() {
  ...
  logger.logPerformanceMetric("ai.service.initialization", initDuration, "ms")
