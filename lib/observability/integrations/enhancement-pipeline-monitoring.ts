...
import type {
  ...
  EnhancementContext,
} from "@/lib/enhancement/pipeline/types"
...
export function monitorEnhancementPipeline() {
  return {
    ...
    onPipelineComplete(context: EnhancementContext, duration: number) {
      ...
      logger.logPerformanceMetric("pipeline.total.duration", duration, "ms", {
        enhancement_id: context.enhancementId,
...
export async function generatePipelinePerformanceReport(
  ...
  if (totalMetrics.length > 0) {
    const completed =
      totalMetrics.find((m) => m.labels.status === "completed")?.value || 0
    const failed =
      totalMetrics.find((m) => m.labels.status === "failed")?.value || 0
...
export function monitorPipelineResources(enhancementId: string) {
  ...
  return {
    recordResourceUsage(stage: string) {
      ...
      if (memoryDelta.heapUsed > 500 * 1024 * 1024) {
        // 500MB
        logger.warn(
          {
