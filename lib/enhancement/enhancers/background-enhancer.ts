...
export class BackgroundEnhancer extends BaseEnhancer {
  ...
  private readonly patterns = {
    subtle: ["dots", "grid", "lines", "waves", "circles"],
    geometric: ["triangles", "hexagons", "diamonds", "squares", "polygons"],
    organic: ["blobs", "clouds", "bubbles", "leaves", "curves"],
  }
  ...
  constructor() {
    super(
      ...
      "Adds or improves background elements for visual interest",
      "medium"
    )
  ...
  async analyze(
    ...
    if (needsBackground) {
      const backgroundStrategy = await this.generateBackgroundStrategy(
        analysis,
        preferences
      )
    ...
    if (analysis.engagement.visualAppeal < 50) {
      const patternStrategy = await this.generatePatternStrategy(
        analysis,
        preferences
      )
  ...
  private async generateBackgroundStrategy(
    ...
    const backgroundValue = await this.generateBackgroundValue(
      backgroundType,
      style,
      analysis
    )
  ...
  private selectGradient(
    style: EnhancementPreferences["style"]
  ): BackgroundEnhancement["value"] {
    const gradientSet =
      this.gradients[style as keyof typeof this.gradients] ||
      this.gradients.modern
  ...
  private async generateImageBackground(
    ...
    try {
      ...
      const generatedImage =
        await this.imageGenerationService.generateBackground(request)
  ...
  private mapToImageStyle(
    style?: EnhancementPreferences["style"]
  ): "realistic" | "artistic" | "minimalist" | "professional" | "playful" {
    switch (style) {
      case "modern":
        return "artistic"
      case "classic":
