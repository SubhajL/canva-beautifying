...
export class TypographyEnhancer extends BaseEnhancer {
  ...
  constructor() {
    super(
      ...
      "Improves font selection, hierarchy, and readability",
      "high"
    )
  }
  ...
  async analyze(
    ...
    if (typographyScore < 85) {
      const typographyStrategy = await this.generateTypographyStrategy(
        analysis,
        preferences
      )
    ...
    if (analysis.engagement.readability < 70) {
      const readabilityStrategy =
        await this.generateReadabilityStrategy(analysis)
  ...
  private async generateReadabilityStrategy(
    ...
    const largerBaseSize = Math.max(
      16,
      this.calculateOptimalBaseSize(analysis) + 2
    )
    ...
    const enhancement: TypographyEnhancement = {
      ...
      sizes: {
        ...
        headings: this.generateHeadingSizes(
          largerBaseSize,
          this.scaleRatios.majorThird
        ),
      },
      improvements: {
    ...
    return {
      ...
      description:
        "Improve text readability with larger sizes and better spacing",
      priority: "high",
      impact: 85,
      changes: {
        typography: enhancement,
  ...
  private selectFontPairing(style: EnhancementPreferences["style"]): {
    heading: string
    body: string
    accent?: string
  } {
    // Filter pairings by style
    const stylePairings = this.fontPairings.filter(
      (p) => style === "modern" || p.style === style
    )
  ...
  private selectTypeScale(analysis: DocumentAnalysis): {
    name: string
    ratio: number
  } {
    const hierarchyLevels = analysis.typography.issues.includes(
      "Poor hierarchy"
    )
      ? 5
      : 3
  ...
  private selectReadableFonts(currentFonts: string[]): {
    ...
    const hasSerif = currentFonts.some((f) =>
      ["Georgia", "Times", "Playfair", "Merriweather"].some((serif) =>
        f.includes(serif)
      )
    )
