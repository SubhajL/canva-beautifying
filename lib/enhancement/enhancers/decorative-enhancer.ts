...
export class DecorativeEnhancer extends BaseEnhancer {
  ...
  constructor() {
    super(
      "Decorative Elements",
      "Adds visual interest with shapes, icons, and decorative elements",
      "medium"
    )
  ...
  async analyze(
    ...
    if (analysis.engagement.visualAppeal < 60) {
      const decorativeStrategy = await this.generateDecorativeStrategy(
        analysis,
        preferences
      )
  ...
  private async generateDecorativeStrategy(
    ...
    const elements = await this.generateDecorativeElements(
      style,
      documentType,
      analysis
    )
  ...
  private async generateDecorativeElements(
    ...
    const iconSet =
      this.icons[documentType as keyof typeof this.icons] || this.icons.general
  ...
  private generateEmphasisElements(
    analysis: DocumentAnalysis
  ): DecorativeElement[] {
    const elements: DecorativeElement[] = []
  ...
  private generateCornerDecorations(
    style: EnhancementPreferences["style"]
  ): DecorativeElement[] {
  ...
  private generateSectionDividers(
    style: EnhancementPreferences["style"]
  ): DecorativeElement[] {
    ...
    for (let index = 0; index < selectedIcons.length; index++) {
      ...
      try {
        ...
        const generatedImage =
          await this.imageGenerationService.generateDecorativeElement(request)
  ...
  private generateBackgroundShapes(
    style: EnhancementPreferences["style"]
  ): DecorativeElement[] {
    const shapes: DecorativeElement[] = []
    const shapeTypes =
      style === "playful" ? ["circle", "star"] : ["hexagon", "square"]
