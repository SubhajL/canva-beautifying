...
export class EnhancementService {
  ...
  private async storeEnhancementResult(
    ...
    await supabase.from("enhancements").update(updateData).eq("id", documentId)
  }
  ...
  async listUserEnhancements(userId: string): Promise<
    Array<{
      id: string
      originalUrl: string
      enhancedUrl?: string
  ...
  > {
    ...
    const { data: enhancements } = await supabase
      .from("enhancements")
      .select(
        "id, original_url, enhanced_url, status, created_at, completed_at"
      )
      .eq("user_id", userId)
