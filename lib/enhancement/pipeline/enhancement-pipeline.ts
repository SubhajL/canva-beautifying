...
export class EnhancementPipeline extends EventEmitter {
  ...
  async execute(): Promise<CompositionResult> {
    try {
      ...
      const analysisResult = await this.runStage(
        "initial-analysis",
        async () => {
      ...
      const enhancementPlan = await this.runStage(
      ...
      const compositionResult = await this.runStage(
        "final-composition",
        async () => {
  ...
  private async savePipelineResults(result: CompositionResult) {
    ...
    await supabase.from("enhancement_pipelines").insert({
      ...
      stages_completed: Object.keys(this.state.stages).filter(
        (key) =>
          this.state.stages[key as keyof PipelineState["stages"]]?.status ===
          "completed"
      ),
      processing_time: result.processingTime.total,
      quality_improvement:
        result.improvements.overallScore.after -
        result.improvements.overallScore.before,
