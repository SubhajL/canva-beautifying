...
export class AssetGenerationStage {
  ...
  async execute(
    ...
    try {
      ...
      if (plan.assetRequirements.decorativeElements.length > 0) {
        generatedAssets.decorativeElements =
          await this.generateDecorativeElements(
            context,
      ...
      if (plan.assetRequirements.educationalGraphics.length > 0) {
        generatedAssets.educationalGraphics =
          await this.generateEducationalGraphics(
            context,
      ...
      if (this.analysisResult && context.subscriptionTier !== "free") {
        ...
        if (ageEnhancements.decorativeElements.length > 0) {
          generatedAssets.decorativeElements.push(
            ...ageEnhancements.decorativeElements
          )
        }
      }
      ...
      throw new Error(
        `Asset generation failed: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
          ...
          switch (requirement.style) {
            ...
            case "image":
              ...
              if (context.subscriptionTier === "premium") {
                ...
                backgroundBuffer = await this.downloadAndProcessImage(
                  result.url
                )
              } else {
      ...
      try {
        ...
        for (let i = 0; i < requirement.quantity; i++) {
          ...
          elements.push({
            ...
            rotation:
              requirement.placement === "random" ? Math.random() * 360 : 0,
            metadata, // Add metadata from enhanced generation
          })
        }
      } catch (error) {
  ...
  private async createBorderElement(
    ...
    return await sharp(Buffer.from(svg)).resize(200, 10).png().toBuffer()
  }
  ...
  private detectAgeGroup(): string {
    ...
    const readability =
      this.analysisResult.contentAnalysis?.readabilityScore || 50
    const visualComplexity =
      this.analysisResult.visualHierarchy?.complexityScore || 0.5
  ...
  private async generateAgeAppropriateEnhancements(
    ...
    try {
      ...
      if (plan.assetRequirements.backgrounds.length < 2) {
        ...
        const bgBuffer = await sharp(Buffer.from(bgResult.svg)).png().toBuffer()
