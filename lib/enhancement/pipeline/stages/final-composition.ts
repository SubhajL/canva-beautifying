...
export class FinalCompositionStage {
  async execute(
    ...
    try {
      ...
      throw new Error(
        `Final composition failed: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  private async composeImage(
    ...
    if (
      ...
      analysis.extractedText.headings.length > 0
    ) {
    ...
    if (compositionResult.balance.overall < 0.7) {
      this.applyBalanceCorrection(ctx, compositionResult.balance, {
        width,
        height,
      })
    }
  ...
  private async applyBackgroundToPDFPage(
    page: any,
    _background: GeneratedAssets["backgrounds"][0]
  ) {
  ...
  private applyColorsToPDFPage(
    _page: any,
    _colors: EnhancementPlan["colorEnhancements"]
  ) {
  ...
  private async applyTypographyToPDFPage(
    _page: any,
  ...
  private async addDecorativeElementsToPDFPage(
    page: any,
    elements: GeneratedAssets["decorativeElements"],
    isFirstPage: boolean
  ) {
  ...
  private async placeEducationalGraphics(
    ...
    for (const graphic of graphics) {
      try {
        ...
        const contentSections = layout.sections.filter(
          (s) => s.type === "content"
        )
        if (contentSections.length > 0) {
          ...
          if (graphic.caption) {
            ctx.font = "14px Arial"
            ctx.fillStyle = "#666"
            ctx.fillText(graphic.caption, x, y + graphic.dimensions.height + 20)
          }
        }
      } catch (error) {
  ...
  private getAppliedEnhancements(
    ...
    if (assets) {
      ...
      if (assets.decorativeElements.length > 0) {
        enhancements.push(
          `${assets.decorativeElements.length} decorative element(s)`
        )
      }
      if (assets.educationalGraphics.length > 0) {
        enhancements.push(
          `${assets.educationalGraphics.length} graphic(s) generated`
        )
      }
    }
  ...
  private async createDecorativeLayers(
    ...
    const elementsByType = decorativeElements.reduce(
      (acc, elem) => {
    ...
    for (const [type, elements] of Object.entries(elementsByType)) {
      ...
      for (const element of elements) {
        ...
        if (!position || (position.x === 0 && position.y === 0)) {
          const placement =
            compositionAlgorithms.SmartPlacement.findOptimalPlacement(
              {
  ...
  private getPreferredZonesForType(
    type: string
