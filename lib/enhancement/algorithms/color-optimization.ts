...
export class ColorOptimization {
  ...
  private static harmonizeByHueShift(
    colors: string[],
    base: chroma.Color
  ): string[] {
  ...
  private static harmonizeBySaturation(
    colors: string[],
    base: chroma.Color
  ): string[] {
  ...
  static ensureAccessibility(
    ...
    Object.entries(palette.semantic).forEach(([key, color]) => {
      ...
      if (contrast < minRatio) {
        ...
        palette.semantic[key as keyof typeof palette.semantic] =
          fixed.foreground
    ...
    for (const type of types) {
      ...
      for (let i = 0; i < simulated.length; i++) {
        for (let j = i + 1; j < simulated.length; j++) {
          ...
          if (distance < 20) {
            issues.push(
              `Colors ${i + 1} and ${j + 1} are too similar for ${type}`
            )
            suggestions.push(
              `Increase hue or luminance difference between colors ${i + 1} and ${j + 1}`
            )
          }
        }
      }
