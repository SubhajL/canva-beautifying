...
export class TypographyImprovement {
  ...
  static suggestFontPairings(
    ...
    const primaryChar =
      FONT_CHARACTERISTICS[primaryFont as keyof typeof FONT_CHARACTERISTICS]
    ...
    const suggestions =
      FONT_PAIRINGS[primaryFont as keyof typeof FONT_PAIRINGS] || []

    // Score each pairing
    suggestions.forEach((secondary) => {
      const secondaryChar =
        FONT_CHARACTERISTICS[secondary as keyof typeof FONT_CHARACTERISTICS]
      if (!secondaryChar) return

      const score = this.scorePairing(
        primaryChar,
        secondaryChar,
        style,
        purpose
      )
      const rationale = this.getPairingRationale(
        primaryChar,
        secondaryChar,
        style
      )
    ...
    return pairings.sort((a, b) => b.score - a.score).slice(0, count)
  }
  ...
  private static scorePairing(
    ...
    switch (purpose) {
      case "heading-body":
        if (primary.category === "serif" && secondary.category === "sans-serif")
          score += 20
        if (primary.category === "sans-serif" && secondary.category === "serif")
          score += 20
        break

      case "display-text":
        if (
          primary.personality === "elegant" ||
          primary.personality === "playful"
        )
          score += 15
      ...
      case "ui":
        if (
          primary.readability === "excellent" &&
          secondary.readability === "excellent"
        )
          score += 30
        if (
          primary.category === "sans-serif" &&
          secondary.category === "sans-serif"
        )
          score += 10
        break
    }
    ...
    score +=
      (readabilityScore[primary.readability as keyof typeof readabilityScore] ||
        0) / 2
    score +=
      (readabilityScore[
        secondary.readability as keyof typeof readabilityScore
      ] || 0) / 2

    return Math.min(100, score)
  }

  private static getPairingRationale(
    primary: any,
    secondary: any,
    _style: string
  ): string {
    const contrastRationale =
      primary.category !== secondary.category
        ? `${primary.category} and ${secondary.category} create visual contrast`
        : `Both ${primary.category} fonts create consistency`

    const personalityRationale =
  ...
  private static getDefaultPairings(
    purpose: string,
    count: number
  ): FontPairing[] {
    const defaults = [
      {
        ...
        score: 90,
        rationale: "Classic sans-serif and serif combination",
      },
      {
        primary: "Roboto",
        secondary: "Roboto Slab",
        score: 85,
        rationale: "Same family provides consistency",
      },
      {
        primary: "Open Sans",
        secondary: "Merriweather",
        score: 88,
        rationale: "Popular and highly readable pairing",
      },
    ]
  ...
  static enhanceReadability(
    ...
    if (currentMetrics.fontSize < 14) {
      recommendations.push(
        "Increase body font size to at least 16px for better readability"
      )
    }
    ...
    if (currentMetrics.lineLength > 75) {
      recommendations.push(
        "Reduce line length to 65-75 characters for optimal reading"
      )
    ...
    // Check line height
    const optimalLineHeight = this.calculateLineHeight(
    ...
    if (Math.abs(currentMetrics.lineHeight - optimalLineHeight) > 0.1) {
      recommendations.push(
        `Adjust line height to ${optimalLineHeight} for better readability`
      )
    }
   ...
   */
  static generateTypographySystem(options: {
    baseSize?: number
    ...
    const fonts = options.primaryFont
      ? {
          heading: options.primaryFont,
          body: options.primaryFont,
          mono: "monospace",
        }
      : fontMap[style]
