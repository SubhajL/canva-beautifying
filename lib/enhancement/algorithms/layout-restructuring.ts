...
export class LayoutRestructuring {
  ...
  static applyGridSystem(
    ...
    if (alignToGrid) {
      ...
      newElements.forEach((element) => {
        ...
        const endColumn = Math.round(
          (relativeX + originalWidth) / (columnWidth + gap)
        )
  ...
  static correctAlignment(
    ...
    guides.forEach((guide) => {
      ...
      guide.elements.forEach((id) => {
        ...
        const oldPos =
          guide.type === "vertical" ? element.bounds.x : element.bounds.y
        ...
        if (Math.abs(oldPos - targetPosition) > 0.1) {
          corrections.push(
            `Aligned ${element.id} to ${guide.type} guide at ${targetPosition}`
          )
        }
      })
    })
  ...
  static optimizeSpacing(
    ...
    groups.forEach((group) => {
      ...
      // Sort by position
      const sortedVertically = [...group].sort(
        (a, b) => a.bounds.y - b.bounds.y
      )
      const sortedHorizontally = [...group].sort(
        (a, b) => a.bounds.x - b.bounds.x
      )

      // Apply vertical spacing
      this.applySpacingMethod(
        sortedVertically,
        "vertical",
        method,
        minSpacing,
  ...
  static analyzeLayout(elements: LayoutElement[]): LayoutAnalysis {
    ...
    const alignmentIssues = this.detectAlignmentIssues(
      elements,
      alignmentGuides
    )
  ...
  private static preserveElementRelationships(
    ...
    originalElements.forEach((original, i) => {
      ...
      if (newEl && newPrev) {
        const originalGap =
          original.bounds.x - (prev.bounds.x + prev.bounds.width)
        const newGap =
          newEl.bounds.x - (newPrev.bounds.x + newPrev.bounds.width)

        if (
          Math.abs(originalGap) < 100 &&
          Math.abs(newGap - originalGap) > 10
        ) {
  ...
  private static detectAlignmentGuides(
    ...
    elements.forEach((el) => {
      ...
      const nearbyX = Array.from(xPositions.keys()).find(
        (pos) => Math.abs(pos - x) <= threshold
      )
    ...
    elements.forEach((el) => {
      ...
      const nearbyY = Array.from(yPositions.keys()).find(
        (pos) => Math.abs(pos - y) <= threshold
      )
  ...
  private static applyOpticalAlignment(
    elements: LayoutElement[],
    corrections: string[]
  ): void {
    ...
    for (let i = 1; i < elements.length; i++) {
      ...
      if (direction === "vertical") {
        ...
        if (Math.abs(current.bounds.x - newX) > 1) {
          ...
          changes.push(
            `Applied ${spacing}px horizontal spacing to ${current.id}`
          )
        }
      }
    }
  ...
  private static applyProportionalSpacing(
    ...
    for (let i = 1; i < elements.length; i++) {
      ...
      changes.push(
        `Applied proportional spacing (${Math.round(spacing)}px) to ${current.id}`
      )
    }
  }
  ...
  private static applyRhythmicSpacing(
    ...
    for (let i = 1; i < elements.length; i++) {
      ...
      changes.push(
        `Applied rhythmic spacing (${multiplier}x = ${spacing}px) to ${current.id}`
      )
    }
  }
  ...
  private static calculateAlignmentScore(
    ...
    const avgElementsPerGuide =
      guides.reduce((sum, g) => sum + g.elements.length, 0) /
      (guides.length || 1)
  ...
  private static detectAlignmentIssues(
    ...
    elements.forEach((el) => {
      ...
      if (nearMisses.length > 0) {
        issues.push(
          `${el.id} is slightly misaligned (off by ${Math.round(nearMisses[0].position)}px)`
        )
      }
    })
  ...
  private static calculateSpacingScore(elements: LayoutElement[]): number {
    ...
    if (stdDev / avgSpacing < 0.3)
      score += 20 // Consistent spacing
  ...
  private static calculateMinSpacing(
    el1: LayoutElement,
    el2: LayoutElement
  ): number {
  ...
  private static calculateSpacingConsistency(
    elements: LayoutElement[]
  ): number {
  ...
  private static calculateBalance(elements: LayoutElement[]): {
    ...
    const horizontalBalance =
      50 +
      50 * (1 - Math.abs(leftWeight - rightWeight) / (leftWeight + rightWeight))
    const verticalBalance =
      50 +
      50 * (1 - Math.abs(topWeight - bottomWeight) / (topWeight + bottomWeight))
  ...
  private static detectFlowPattern(
    elements: LayoutElement[]
  ): VisualFlow["pattern"] {
    ...
    const topLeft = elements.find(
      (el) => el.bounds.x < 200 && el.bounds.y < 200
    )
    const topRight = elements.find(
      (el) => el.bounds.x > 400 && el.bounds.y < 200
    )
    const bottomLeft = elements.find(
      (el) => el.bounds.x < 200 && el.bounds.y > 400
    )
    const bottomRight = elements.find(
      (el) => el.bounds.x > 400 && el.bounds.y > 400
    )
    ...
    switch (pattern) {
      ...
      case "Z":
        ...
        for (let i = 1; i < elements.length; i++) {
          ...
          const current = elements[i]
          if (
            current.bounds.x > prev.bounds.x ||
            current.bounds.y > prev.bounds.y
          ) {
            score += 5
          }
        }
  ...
  private static arrangeForZPattern(
    elements: LayoutElement[],
    improvements: string[]
  ): void {
  ...
  private static arrangeForLinearFlow(
    elements: LayoutElement[],
    improvements: string[]
  ): void {
  ...
  private static emphasizeHierarchy(
    elements: LayoutElement[],
    improvements: string[]
  ): void {
  ...
  private static optimizeReadingPath(
    elements: LayoutElement[],
    improvements: string[]
  ): void {
    // Ensure text elements follow natural reading order
    const textElements = elements.filter(
      (el) => el.type === "text" || el.type === "header"
    )
