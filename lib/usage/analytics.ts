...
export class UsageAnalytics {
  ...
  async analyzeUsagePatterns(userId: string): Promise<UsagePattern> {
    ...
    currentMonthData.forEach((record) => {
      ...
      dayOfWeekUsage.set(
        dayOfWeek,
        (dayOfWeekUsage.get(dayOfWeek) || 0) + credits
      )
    })

    // Calculate average daily usage
    const totalDays = Math.max(dailyUsage.size, 1)
    const totalUsage = Array.from(dailyUsage.values()).reduce(
      (sum, val) => sum + val,
      0
    )
  ...
  async detectAnomalies(userId: string): Promise<{
    ...
    const todayTotal = todayUsage.reduce(
      (sum, r) => sum + (r.credits_used || 1),
      0
    )
    ...
    if (user) {
      ...
      const usagePercentage =
        (user.usage_count / patterns.averageDailyUsage) * 100
      ...
      if (dayOfMonth <= 5 && usagePercentage >= 50) {
        return {
          ...
          description:
            "You've used over 50% of your monthly credits in the first few days",
        }
      }
    }
