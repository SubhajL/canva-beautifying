...
export class PromptEngineer {
  ...
  private initializeTemplates(): Map<string, PromptTemplate> {
    ...
    templates.set("background-modern", {
      ...
      template:
        "A modern abstract background with {colors} gradient, subtle {pattern}, professional design, high quality, 4k resolution",
    ...
    templates.set("background-playful", {
      ...
      template:
        "A playful, colorful background with {theme} theme, fun patterns, child-friendly design, bright {colors} colors, whimsical elements",
    ...
    templates.set("background-professional", {
      ...
      template:
        "A professional, minimalist background with subtle {pattern} pattern, {colors} color scheme, corporate design, clean and elegant",
    ...
    templates.set("element-icon", {
      ...
      name: "Icon Element",
      template:
        "A simple {style} icon of {subject}, minimalist design, vector style, {colors} colors, transparent background",
    ...
    templates.set("element-pattern", {
      ...
      template:
        "A seamless {pattern} pattern, {style} style, {colors} color palette, tileable design, decorative element",
    ...
    templates.set("element-border", {
      ...
      name: "Border Element",
      template:
        "A decorative {style} border frame, {thickness} lines, {colors} colors, ornamental design, transparent center",
  ...
  private selectBackgroundTemplate(
  ...
  suggestPromptImprovements(prompt: string): string[] {
    ...
    if (!hasStyle) {
      suggestions.push(
        "Consider adding style descriptors (modern, vintage, minimalist, etc.)"
      )
    }

    // Check for quality keywords
    const hasQuality = /quality|resolution|detailed|professional/.test(
      prompt.toLowerCase()
    )
    if (!hasQuality) {
      suggestions.push(
        'Add quality modifiers like "high quality" or "professional"'
      )
    }
