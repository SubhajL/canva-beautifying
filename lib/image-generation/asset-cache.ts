...
export class AssetCache {
  ...
  async store(image: GeneratedImage): Promise<void> {
    ...
    this.memoryCache.set(
      this.getCacheKey(image.prompt, image.style),
      cachedAsset
    )
  ...
  private findInMemoryCache(
    ...
    for (const [cachedKey, asset] of this.memoryCache.entries()) {
      if (
        this.isSimilarPrompt(prompt, asset.prompt) &&
        asset.style === (style || "realistic")
      ) {
  ...
  private extractTags(prompt: string): string[] {
    ...
    // Extract color tags
    const colorPattern =
      /(red|blue|green|yellow|purple|orange|pink|black|white|gray)/gi
