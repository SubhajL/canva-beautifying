...
export class StableDiffusionProvider extends BaseImageProvider {
      ...
      try {
        ...
        const input = {
          ...
          negative_prompt:
            ...
            this.getDefaultNegativePrompt(request.style),
          width: size.width,
          height: size.height,
          num_outputs: 1,
          ...
          prompt_strength: 0.8,
          refine:
            request.quality === "hd" ? "expert_ensemble_refiner" : "no_refiner",
          high_noise_frac: 0.8,
          seed: request.seed,
  ...
  private getDefaultNegativePrompt(style?: string): string {
    const base =
      "ugly, blurry, poor quality, distorted, disfigured, low quality, low resolution, jpeg artifacts, watermark, text, logo"
    ...
    return style && styleSpecific[style]
      ? `${base}, ${styleSpecific[style]}`
      : base
  }
